...
outreg=${3:-"${det_lower:1}${expid}_${inreg%.*}_physical"}
...
####
##  Open and save region file to physical with ds9
####

if [ -f "${PWD}/${skyfile}" ] ;then
    ## DS9 export sky regions to physical coords (NOT the same as detector coord system necessary for ESAS pipeline)
    ...
    ds9 "${PWD}/${skyfile}" -region load "${PWD}/${inreg}" -region system physical -region save "${PWD}/${outreg}.reg" -exit &

    wait $!
else
    echo "${skyfile} not found for DS9 export"
fi
...
do
    ...
    phys_x="${line%%","*}"; line="${line#*","}" #trim assigned value from front of line
    phys_x=$(echo "scale=2; $phys_x" | bc -q)
    # if [[ ":" =~ "${ra_hms}" ]] ;then
    ...
    phys_y="${line%%","*}"; line="${line#*","}"
    phys_y=$(echo "scale=2; $phys_y" | bc -q)
    # if [[ ":" =~ "${dec_dms}" ]] ;then
    ...
    if [[ "$shape" == "circle" ]]; then
        ...
        radius="${line%%","*}"
        radius=$(echo "scale=2; $radius" | bc -q)
        ...
        semi_a="${line%%","*}"; line="${line#*","}"
        semi_b="${line%%","*}"; line="${line#*","}"

        semi_a=$(echo "scale=2; $semi_a" | bc -q)
        semi_b=$(echo "scale=2; $semi_b" | bc -q)

        # semi_a_pixels=$(degree2pixel "$semi_a" "$deg_per_pixel")
        # semi_b_pixels=$(degree2pixel "$semi_b" "$deg_per_pixel")
        rotation="${line%%","*}"
        rotation=$(echo "scale=2; $rotation" | bc -q)

        shape_params="$semi_a,$semi_b,$rotation"
        shape_params_xmm="$shape_params"

    elif [[ "$shape" == "box" ]]; then
        ## BOX; WIDTH(as),HEIGHT(as),ROTATION(deg)
        width="${line%%","*}"; line="${line#*","}"
        height="${line%%","*}"; line="${line#*","}"
        ...
        width=$(echo "scale=2; $width/2.0" | bc -q)
        height=$(echo "scale=2; $height/2.0" | bc -q)

        rotation="${line%%","*}"
        rotation=$(echo "scale=2; $rotation" | bc -q)

        shape_params="$width,$height,$rotation"
        shape_params_xmm="$shape_params"
        ...
        while [[ "$line" ]]; do
            ...
            poly_x="${line%%","*}"
            poly_x=$(echo "scale=2; $poly_x" | bc -q)
            # poly_ra=$(ra_hr2deg_decimal "${line%%","*}"); line="${line#*","}"
            poly_y="${line%%","*}"
            poly_y=$(echo "scale=2; $poly_y" | bc -q)
            # poly_dec=$(dec_deg_decimal "${line%%","*}"); line="${line#*","}"

            poly_phys_coords="$poly_x,$poly_y"

            ## esky2det spits outs strings of "*" if it fails to find a proper conversion
            ## If not accounted for the "*" can expand later into a list of files in directory and break code
            ## If found, then skip the current polygon region
            if [[ "$poly_phys_coords" == *"*"* ]]; then
                echo "$line"
                echo "$poly_phys_coords"
                ...
                shape_params="$shape_params$leading_comma$poly_phys_coords"
                shape_params_xmm="$shape_params"
                leading_comma=","
            fi
        done
...
# catfood dog mouse frog food
    ...
    phys_reg_str_xmm="(X,Y) IN $shape($phys_x,$phys_y,$shape_params_xmm)"
    ...
    if [[ $initial_set == true ]]; then
        ...
        phys_reg_str_xmm="&&$sign($phys_reg_str_xmm"
    fi
    ...
    if [[ $initial_set == false && $set_switch == true ]]; then
        ...
        phys_reg_str_xmm="||$phys_reg_str_xmm"
    fi

    initial_set=false

    # det_reg_str_xmm="&&$sign((DETX,DETY) IN $shape($det_x,$det_y,$shape_params_xmm))"
    echo $phys_reg_str_xmm

    echo -n "$phys_reg_str_xmm" >> "$outreg.txt"

    ## Expected input/outputs
    #-circle(9:56:50.9163,+69:50:32.120,111.974")

    #-circle(12422.0,4758.4,2239.4799935820) #ds9 format
    #&&!((X,Y) IN circle(12422.0,4758.4,2239.4799935820)) #xmm format
...
echo
echo WARNING: the following must be less than 1024 bytes or it will NOT work as a selection expression
echo multi-byte char str size: $(wc -m < "$outreg.txt") bytes
echo single-byte char str size: $(wc -c < "$outreg.txt") bytes
echo